...
import { controlPauseMessage, controlStartMessage } from "../../protocol/testMessages";

export type ConversationMode = 'simplex';
...
export const Conversation: FC<ConversationProps> = ({
  ...
  // 发送 system prompt / user instruction 到后端
  useEffect(() => {
    if (!isConnected) {
      return;
    }
    ...
    if (systemPrompt && systemPrompt.trim()) {
      metadata.system_prompt = systemPrompt.trim();
    }
    if (userInstruction && userInstruction.trim()) {
      metadata.instruction = userInstruction.trim();
    }
  ...
  }, [isConnected, systemPrompt, userInstruction, sendMessage]);
  ...
  const togglePause = useCallback(() => {
    if (isPaused) {
      sendMessage(controlStartMessage);
      setIsPaused(false);
    } else {
      sendMessage(controlPauseMessage);
      setIsPaused(true);
    }
