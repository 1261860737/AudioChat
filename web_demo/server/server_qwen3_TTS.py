...
class ServerState:
    ...
    async def handle_chat(self, request):
        ...
        text_log_queue = queue.Queue()

        def enqueue_text_log(path: Optional[str], content: str) -> None:
            if not path or not content:
                return
            try:
                text_log_queue.put((path, content))
            except Exception as exc:
                log("warning", f"Failed to enqueue text log: {exc}")
                ...
                try:
                    ...
                    if signal_type == "pause":
                        ...
                        if all_recorded_pcm is not None and len(all_recorded_pcm) > 0:
                            ...
                            text_filename = f"{client_id}_turn{turn_counter}_output_{text_timestamp}.txt"
                            text_output_path = os.path.join(self.output_dir, "text", text_filename)
                            current_generation["text_output_path"] = text_output_path
                            enqueue_text_log(text_output_path, f"[{text_timestamp}] TURN {turn_counter}\n")
                            enqueue_text_log(text_output_path, "[USER]\n")
                            enqueue_text_log(text_output_path, transcript_text.strip() + "\n\n[ASSISTANT]\n")
                            log("info", f"[Turn {turn_counter}] Text stream log: {text_output_path}")
                            ...
                            async def run_llm_and_tts():
                                ...
                                async def on_delta(delta: str):
                                    nonlocal accumulated_text, pending_tts_text
                                    if current_generation["interrupt"]:
                                        return
                                    accumulated_text += delta
                                    current_generation["accumulated_text"] = accumulated_text
                                    enqueue_text_log(current_generation.get("text_output_path"), delta)
                                    if len(accumulated_text) == len(delta):
                                        log("info", f"[Turn {turn_counter}] First LLM delta received ({len(delta)} chars)")
                                    pending_tts_text += delta
                                    while True:
                                        sentence, pending_tts_text = extract_tts_sentence(pending_tts_text)
                                        if not sentence:
                                            break
                                        tts_text_queue.put((active_turn_id, sentence, False))

                                try:
                                    try:
                                        if self.llm_stream:
                                            llm_text = await vllm_stream_chat_completion(
                                            ...
                                            if not accumulated_text and llm_text:
                                                await on_delta(llm_text)
                                        # else:
                                    ...
                                    # Flush any remaining complete sentences first, then finalize the tail.
                                    while True:
                                        sentence, pending_tts_text = extract_tts_sentence(pending_tts_text)
                                        if not sentence:
                                            break
                                    ...
                                    if pending_tts_text.strip():
                                        tts_text_queue.put((active_turn_id, pending_tts_text, True))
                                    else:
                                    ...
                                    if current_generation.get("text_output_path"):
                                        enqueue_text_log(current_generation.get("text_output_path"), "\n[END]\n")
                                        log("info", f"[Turn {turn_counter}] Text stream saved ({len(accumulated_text)} chars)")
                                    current_generation["text_output_path"] = ""
                                    if is_processing:
                                        is_processing = False
        ...
        def text_log_thread_func():
            """文本落盘线程：异步写入中间文本（避免阻塞事件循环）。"""
            log("info", "Text log thread started")
            while True:
                try:
                    item = text_log_queue.get(timeout=0.1)
                except queue.Empty:
                    continue
                if item is None:
                    break
                ...
                try:
                    ...
                    log("warning", f"Failed to write text log: {exc}")
            log("info", "Text log thread stopped")
        ...
        text_log_thread = Thread(target=text_log_thread_func, daemon=True)

        tts_worker_thread.start()
        encode_thread.start()
        text_log_thread.start()

        log("info", "All workers started (Qwen3-TTS worker + encode thread + text log active)")
        # 发送握手包：告知协议版本与模型类型
        ...
        try:
            ...
            text_log_queue.put(None)
            text_log_thread.join(timeout=2.0)
