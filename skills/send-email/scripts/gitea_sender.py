...
def _maybe_load_env_file() -> None:
    """在缺少环境变量时，尝试从 `skills/send-email/.env` 加载（按当前写法 `KEY=VALUE`）。

    说明：
    - 本仓库的测试脚本 `skills/send-email/scripts/main.py` 直接依赖环境变量。
    ...
    skill_dir = Path(__file__).resolve().parents[1]  # .../skills/send-email
