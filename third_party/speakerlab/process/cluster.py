# Copyright 3D-Speaker (https://github.com/alibaba-damo-academy/3D-Speaker). All Rights Reserved.
# Licensed under the Apache License, Version 2.0 (http://www.apache.org/licenses/LICENSE-2.0)

"""
cluster.py：说话人聚类模块

核心功能：
- 输入：embedding 矩阵 X，形状通常为 [N, D]（N=片段数，D=嵌入维度）
- 输出：labels，形状为 [N]，每个样本对应的簇ID（说话人ID）

在说话人分离（Speaker Diarization）中常见流程：
1) 对语音片段抽取 embedding
2) 使用本模块聚类得到每段说话人标签
3) 结合时间信息合并/后处理得到最终分离结果

本文件包含多种聚类实现，并提供统一封装（CommonClustering），以及音视频联合聚类（JointClustering）。
"""

import numpy as np
import scipy
import sklearn
from sklearn.cluster._kmeans import k_means
from sklearn.metrics.pairwise import cosine_similarity

# try:
#     import fastcluster
# except Exception:
#     fastcluster = None

from scipy.cluster.hierarchy import linkage as scipy_linkage

from scipy.cluster.hierarchy import fcluster
from scipy.spatial.distance import squareform

try:
    import umap, hdbscan
except ImportError:
    raise ImportError(
        'Package "umap" or "hdbscan" not found. \
        Please install them first by "pip install umap-learn hdbscan".'
    )


class SpectralCluster:
    """谱聚类（Spectral Clustering）

    基于相似度矩阵（affinity matrix）的谱聚类实现，使用非归一化拉普拉斯矩阵 L = D - A。
    本实现改编自 https://github.com/speechbrain/speechbrain。

    处理流程：
    1) 计算余弦相似度矩阵 A
    2) p_pruning：按行裁剪弱连接（将较小相似度置 0，使图更稀疏）
    3) 对称化：A = 0.5 * (A + A.T)
    4) 构建拉普拉斯矩阵：L = D - A
    5) 对 L 做特征分解，得到谱嵌入，并确定说话人数量 k
       - 若给定 oracle_num（speaker_num），直接用它
       - 否则用 EigenGap（特征值间隙）在 [min_num_spks, max_num_spks] 内选择 k
    6) 在谱嵌入空间执行 k-means 聚类得到 labels
    """

    def __init__(
        self,
        min_num_spks=1,
        max_num_spks=10,
        pval=0.02,
        min_pnum=6,
        oracle_num=None,
    ):
        """初始化谱聚类器

        Args:
            min_num_spks: 最少说话人数量
            max_num_spks: 最多说话人数量
            pval: 相似度矩阵裁剪比例/强度（越大裁剪越多）
            min_pnum: 防止裁剪过度的保护项
            oracle_num: 预设说话人数量（已知时可指定）
        """
        self.min_num_spks = min_num_spks
        self.max_num_spks = max_num_spks
        self.min_pnum = min_pnum
        self.pval = pval
        self.k = oracle_num

    def __call__(self, X, **kwargs):
        """执行谱聚类

        Args:
            X: [N, D] 的 embedding 矩阵
            kwargs:
                pval: 可覆盖默认 self.pval
                speaker_num: 预设说话人数量（oracle）

        Returns:
            labels: [N]，每个 embedding 的簇标签（从 0 开始）
        """
        pval = kwargs.get("pval", None)
        oracle_num = kwargs.get("speaker_num", None)

        # 计算相似度矩阵
        sim_mat = self.get_sim_mat(X)

        # 按 pval 裁剪相似度矩阵
        prunned_sim_mat = self.p_pruning(sim_mat, pval)

        # 对称化处理
        sym_prund_sim_mat = 0.5 * (prunned_sim_mat + prunned_sim_mat.T)

        # 计算拉普拉斯矩阵
        laplacian = self.get_laplacian(sym_prund_sim_mat)

        # 获取谱嵌入
        emb, num_of_spk = self.get_spec_embs(laplacian, oracle_num)

        # 执行聚类
        labels = self.cluster_embs(emb, num_of_spk)

        return labels

    def get_sim_mat(self, X):
        """计算余弦相似度矩阵

        Args:
            X: [N, D]

        Returns:
            M: [N, N]，两两余弦相似度矩阵
        """
        # 余弦相似度
        M = cosine_similarity(X, X)
        return M

    def p_pruning(self, A, pval=None):
        """按 p-value 裁剪相似度矩阵（注意：会原地修改 A）

        对相似度矩阵的每一行，将较小相似度置 0，保留更强的连接以降低噪声边。

        Args:
            A: [N, N] 相似度矩阵（会被原地修改）
            pval: 裁剪比例，None 时使用 self.pval

        Returns:
            A: 裁剪后的相似度矩阵
        """
        if pval is None:
            pval = self.pval
        n_elems = int((1 - pval) * A.shape[0])
        n_elems = min(n_elems, A.shape[0] - self.min_pnum)

        # 遍历相似度矩阵的每一行
        for i in range(A.shape[0]):
            low_indexes = np.argsort(A[i, :])
            low_indexes = low_indexes[0:n_elems]

            # 将较小的相似度置为 0
            A[i, low_indexes] = 0
        return A

    def get_laplacian(self, M):
        """构建非归一化拉普拉斯矩阵 L = D - A

        Args:
            M: [N, N] 对称相似度矩阵

        Returns:
            L: [N, N] 拉普拉斯矩阵
        """
        M = M.copy()
        np.fill_diagonal(M, 0)
        D = np.sum(np.abs(M), axis=1)

        L = -M
        L[np.diag_indices_from(L)] = D
        return L

    def get_spec_embs(self, L, k_oracle=None):
        """获取谱嵌入并确定聚类数 k

        Args:
            L: [N, N] 拉普拉斯矩阵
            k_oracle: 预设说话人数量（oracle），None 时自动估计

        Returns:
            emb: [N, k] 谱嵌入
            num_of_spk: 说话人数量 k
        """
        if k_oracle is None:
            k_oracle = self.k

        lambdas, eig_vecs = scipy.sparse.linalg.eigsh(
            L, k=min(self.max_num_spks + 1, L.shape[0]), which="SM"
        )

        if k_oracle is not None:
            num_of_spk = k_oracle
        else:
            lambda_gap_list = self.getEigenGaps(
                lambdas[self.min_num_spks - 1 : self.max_num_spks + 1]
            )
            num_of_spk = np.argmax(lambda_gap_list) + self.min_num_spks

        emb = eig_vecs[:, :num_of_spk]
        return emb, num_of_spk

    def cluster_embs(self, emb, k):
        """对谱嵌入做 k-means 聚类

        Args:
            emb: [N, k] 谱嵌入
            k: 聚类数（说话人数量）

        Returns:
            labels: [N] 聚类标签
        """
        # k-means 聚类
        _, labels, _ = k_means(emb, k)
        return labels

    def getEigenGaps(self, eig_vals):
        """计算特征值间隙（EigenGap）

        Args:
            eig_vals: 特征值列表

        Returns:
            eig_vals_gap_list: 相邻特征值差值列表
        """
        eig_vals_gap_list = []
        for i in range(len(eig_vals) - 1):
            gap = float(eig_vals[i + 1]) - float(eig_vals[i])
            eig_vals_gap_list.append(gap)
        return eig_vals_gap_list


class UmapHdbscan:
    """UMAP 降维 + HDBSCAN 密度聚类

    参考：
    - Siqi Zheng, Hongbin Suo. Reformulating Speaker Diarization as Community Detection With
      Emphasis On Topological Structure. ICASSP2022

    流程：
    1) UMAP 将高维 embedding 降到较低维空间
    2) HDBSCAN 做密度聚类（无需指定簇数，可能输出 -1 表示噪声点）
    """

    def __init__(
        self,
        n_neighbors=20,
        n_components=60,
        min_samples=20,
        min_cluster_size=10,
        metric="euclidean",
    ):
        """初始化 UMAP + HDBSCAN 聚类器"""
        self.n_neighbors = n_neighbors
        self.n_components = n_components
        self.min_samples = min_samples
        self.min_cluster_size = min_cluster_size
        self.metric = metric

    def __call__(self, X, **kwargs):
        """执行 UMAP + HDBSCAN 聚类并返回 labels"""
        umap_X = umap.UMAP(
            n_neighbors=self.n_neighbors,
            min_dist=0.0,
            n_components=min(self.n_components, X.shape[0] - 2),
            metric=self.metric,
        ).fit_transform(X)
        labels = hdbscan.HDBSCAN(
            min_samples=self.min_samples, min_cluster_size=self.min_cluster_size
        ).fit_predict(umap_X)
        return labels


class AHCluster:
    """凝聚层次聚类（Agglomerative Hierarchical Clustering, AHC）

    自底向上迭代合并最近的簇，直到满足阈值条件。
    本实现改编自 https://github.com/BUTSpeechFIT/VBx。

    关键点：
    - 用余弦相似度构建相似度矩阵
    - 将相似度转换为距离（取负号）并用 squareform 压缩
    - fastcluster.linkage(method='average') 构建层次树
    - fcluster 用阈值切树得到 labels
    """

    def __init__(self, fix_cos_thr=0.4):
        """初始化 AHC

        Args:
            fix_cos_thr: 固定余弦相似度阈值（越大越容易合并，簇数越少）
        """
        self.fix_cos_thr = fix_cos_thr

    def __call__(self, X, **kwargs):
        """执行凝聚层次聚类并返回 labels"""
        scr_mx = cosine_similarity(X)
        scr_mx = squareform(-scr_mx, checks=False)
        # if fastcluster is not None:
        #     lin_mat = fastcluster.linkage(scr_mx, method="average", preserve_input="False")
        # else:
        lin_mat = scipy_linkage(scr_mx, method="average")
        adjust = abs(lin_mat[:, 2].min())
        lin_mat[:, 2] += adjust
        labels = fcluster(lin_mat, -self.fix_cos_thr + adjust, criterion="distance") - 1
        return labels


class CommonClustering:
    """通用聚类封装：统一入口 + 后处理

    功能：
    1) 支持多种聚类算法：spectral / umap_hdbscan / AHC
    2) 对短序列（N 较小）优先使用 cluster_for_short（默认 AHC）
    3) 后处理：
       - filter_minor_cluster：将小簇（样本数 <= min_cluster_size）吸附到最近大簇
       - merge_by_cos：簇中心余弦相似度超过阈值则迭代合并（解决过分割）
    """

    def __init__(
        self,
        cluster_type,
        cluster_line=40,
        mer_cos=None,
        min_cluster_size=4,
        **kwargs,
    ):
        """初始化通用聚类器"""
        self.cluster_type = cluster_type
        self.cluster_line = cluster_line
        self.min_cluster_size = min_cluster_size
        self.mer_cos = mer_cos
        if self.cluster_type == "spectral":
            self.cluster = SpectralCluster(**kwargs)
        elif self.cluster_type == "umap_hdbscan":
            kwargs["min_cluster_size"] = min_cluster_size
            self.cluster = UmapHdbscan(**kwargs)
        elif self.cluster_type == "AHC":
            self.cluster = AHCluster(**kwargs)
        else:
            raise ValueError("%s is not currently supported." % self.cluster_type)
        if self.cluster_type != "AHC":
            self.cluster_for_short = AHCluster()
        else:
            self.cluster_for_short = self.cluster

    def __call__(self, X, **kwargs):
        """执行聚类并返回 labels（包含后处理）

        Args:
            X: [N, D] embedding 矩阵
            kwargs: 传递给底层聚类器的参数（如 speaker_num、pval 等）

        Returns:
            labels: [N] 聚类标签
        """
        # 聚类并返回标签
        assert len(X.shape) == 2, "Shape of input should be [N, C]"
        if X.shape[0] <= 1:
            return np.zeros(X.shape[0], dtype=int)
        if X.shape[0] < self.cluster_line:
            labels = self.cluster_for_short(X)
        else:
            labels = self.cluster(X, **kwargs)

        # 将极小簇吸附到主要簇
        labels = self.filter_minor_cluster(labels, X, self.min_cluster_size)
        # 合并相似说话人簇
        if self.mer_cos is not None:
            labels = self.merge_by_cos(labels, X, self.mer_cos)

        return labels

    def filter_minor_cluster(self, labels, x, min_cluster_size):
        """过滤极小簇

        将样本数 <= min_cluster_size 的簇视为小簇，
        对小簇内的每个样本，计算其与各主要簇中心的余弦相似度，
        并将其重新分配到最相似的主要簇。
        """
        cset = np.unique(labels)
        csize = np.array([(labels == i).sum() for i in cset])
        minor_idx = np.where(csize <= min_cluster_size)[0]
        if len(minor_idx) == 0:
            return labels

        minor_cset = cset[minor_idx]
        major_idx = np.where(csize > min_cluster_size)[0]
        if len(major_idx) == 0:
            return np.zeros_like(labels)
        major_cset = cset[major_idx]
        major_center = np.stack([x[labels == i].mean(0) for i in major_cset])
        for i in range(len(labels)):
            if labels[i] in minor_cset:
                cos_sim = cosine_similarity(x[i][np.newaxis], major_center)
                labels[i] = major_cset[cos_sim.argmax()]

        return labels

    def merge_by_cos(self, labels, x, cos_thr):
        """基于簇中心余弦相似度合并簇

        迭代寻找最相似的簇中心对：
        - 若最大相似度 >= cos_thr，则合并该簇对
        - 否则停止

        用于缓解过分割（同一说话人被分成多个簇）。
        """
        # 基于簇中心余弦相似度合并簇
        assert cos_thr > 0 and cos_thr <= 1
        while True:
            cset = np.unique(labels)
            if len(cset) == 1:
                break
            centers = np.stack([x[labels == i].mean(0) for i in cset])
            affinity = cosine_similarity(centers, centers)
            affinity = np.triu(affinity, 1)
            idx = np.unravel_index(np.argmax(affinity), affinity.shape)
            if affinity[idx] < cos_thr:
                break
            c1, c2 = cset[np.array(idx)]
            labels[labels == c2] = c1
        return labels


class JointClustering:
    """音视频联合聚类（多模态说话人分离）

    思路：
    1) 分别对音频 embedding 与视觉 embedding 做聚类，得到 alabels / vlabels
    2) 将视觉标签整理成连续时间区间 vlist
    3) 使用时间重叠关系找到每个音频簇对应的视觉说话人集合，并修正音频标签：
       - 如果一个音频簇与多个视觉说话人重叠：用 embedding 相似度分配到不同视觉说话人
       - 如果只与一个视觉说话人重叠：直接替换标签
    """

    def __init__(self, audio_cluster, vision_cluster):
        self.audio_cluster = audio_cluster
        self.vision_cluster = vision_cluster

    def __call__(self, audioX, visionX, audioT, visionT, conf):
        # 分别做音频聚类与视觉聚类
        alabels = self.audio_cluster(audioX)
        vlabels = self.vision_cluster(visionX)

        alabels = self.arrange_labels(alabels)
        vlist, vspk_embs, vspk_dur = self.get_vlist_embs(
            audioX, alabels, vlabels, audioT, visionT, conf
        )

        # 用视觉聚类结果修正音频标签
        aspk_num = alabels.max() + 1
        for i in range(aspk_num):
            aspki_index = np.where(alabels == i)[0]
            aspki_embs = audioX[alabels == i]

            aspkiT_part = np.array(audioT)[alabels == i]
            overlap_vspk = self.overlap_spks(
                self.cast_overlap(aspkiT_part), vlist, vspk_dur
            )
            if len(overlap_vspk) > 1:
                centers = np.stack([vspk_embs[s] for s in overlap_vspk])
                distribute_labels = self.distribute_embs(aspki_embs, centers)
                for j in range(distribute_labels.max() + 1):
                    for loc in aspki_index[distribute_labels == j]:
                        alabels[loc] = overlap_vspk[j]
            elif len(overlap_vspk) == 1:
                for loc in aspki_index:
                    alabels[loc] = overlap_vspk[0]

        alabels = self.arrange_labels(alabels)
        return alabels

    def overlap_spks(self, times, vlist, vspk_dur=None):
        # 找出与给定时间段重叠的视觉说话人
        overlap_dur = {}
        for [a_st, a_ed] in times:
            for [v_st, v_ed, v_id] in vlist:
                if a_ed > v_st and v_ed > a_st:
                    if v_id not in overlap_dur:
                        overlap_dur[v_id] = 0
                    overlap_dur[v_id] += min(a_ed, v_ed) - max(a_st, v_st)
        vspk_list = []
        for v_id, dur in overlap_dur.items():
            # 重叠确认阈值
            if (vspk_dur is None and dur > 0.5) or (
                vspk_dur is not None and dur > min(vspk_dur[v_id] * 0.5, 0.5)
            ):
                vspk_list.append(v_id)
        return vspk_list

    def distribute_embs(self, embs, centers):
        # embs: [n, D]，centers: [k, D]
        norm_centers = centers / np.linalg.norm(centers, axis=1, keepdims=True)
        norm_embs = embs / np.linalg.norm(embs, axis=1, keepdims=True)
        similarity = np.matmul(norm_embs, norm_centers.T)  # [n, k]
        argsort = np.argsort(similarity, axis=-1)
        return argsort[:, -1]

    def get_vlist_embs(self, audioX, alabels, vlabels, audioT, visionT, conf):
        assert len(vlabels) == len(visionT)
        vlist = []
        for i, ti in enumerate(visionT):
            if (
                len(vlist) == 0
                or vlabels[i] != vlist[-1][2]
                or ti - visionT[i - 1] > conf.face_det_stride * 0.04 + 1e-4
            ):
                if len(vlist) > 0 and vlist[-1][1] - vlist[-1][0] < 1e-4:
                    # 移除过短区间
                    vlist.pop()
                vlist.append([ti, ti, vlabels[i]])
            else:
                vlist[-1][1] = ti

        # 整理/重编号视觉标签
        vlabels_arrange = self.arrange_labels(
            [i[2] for i in vlist], a_st=alabels.max() + 1
        )
        vlist = [[i[0], i[1], j] for i, j in zip(vlist, vlabels_arrange)]

        # 收集与 vlist 对齐的音频 embedding
        vspk_embs = {}
        for [v_st, v_ed, v_id] in vlist:
            for i, [a_st, a_ed] in enumerate(audioT):
                if a_ed >= v_st and v_ed >= a_st:
                    if min(a_ed, v_ed) - max(a_st, v_st) > 1:
                        if v_id not in vspk_embs:
                            vspk_embs[v_id] = []
                        vspk_embs[v_id].append(audioX[i])
        for k in vspk_embs:
            vspk_embs[k] = np.stack(vspk_embs[k]).mean(0)

        vlist_new = []
        for i in vlist:
            if i[2] in vspk_embs:
                vlist_new.append(i)
        # 统计每个视觉说话人的持续时间
        vspk_dur = {}
        for i in vlist_new:
            if i[2] not in vspk_dur:
                vspk_dur[i[2]] = 0
            vspk_dur[i[2]] += i[1] - i[0]

        return vlist_new, vspk_embs, vspk_dur

    def cast_overlap(self, input_time):
        if len(input_time) == 0:
            return input_time
        output_time = []
        for i in range(0, len(input_time) - 1):
            if i == 0 or output_time[-1][1] < input_time[i][0]:
                output_time.append(input_time[i])
            else:
                output_time[-1][1] = input_time[i][1]
        return output_time

    def arrange_labels(self, labels, a_st=0):
        # 将标签重映射为从 0 开始的连续编号
        new_labels = []
        labels_dict = {}
        idx = a_st
        for i in labels:
            if i not in labels_dict:
                labels_dict[i] = idx
                idx += 1
            new_labels.append(labels_dict[i])
        return np.array(new_labels)
